...
//! let r = parse_hints(&p, "b")?.labels();
//! // ["fruit"]
//! # assert_eq!(r, &["fruit"]);
//!
//! // neither can match, no hints
//! let r = parse_hints(&p, "x");
//! // no hints
...
//!
//! // no input, so both options are possible
//! let r = parse_hints(&p, "")?.replacements();
//! // ["banana", "salmon"]
//! # assert_eq!(r, &["banana", "salmon"]);
//!
//! // only "banana" can match
//! let r = parse_hints(&p, "ba")?.replacements();
//! // ["banana"]
//! # assert_eq!(r, &["banana"]);
//!
//! // neither can match, no hints
...
pub fn map_info<F, R, M>(parser: F, map: M) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
    M: Fn(&mut State),
{
...
/// # Ok::<(), String>(())
/// ```
pub fn label<F, R>(label: &'static str, parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
...
///
pub fn with_label<F, L, R>(label_fn: L, parser: F) -> impl Fn(&str) -> Result<R>
where
    L: Fn() -> Option<Cow<'static, str>>,
    F: Fn(&str) -> Result<R>,
{
...
pub fn unlabel<F, R>(parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
...
pub fn help<F, R>(message: &'static str, parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
...
/// Add a help message to a parser
pub fn optional_help<F, R>(message: Option<&'static str>, parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
    move |input| {
...
pub fn mask_digits<R, F>(parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
...
pub fn mask_digits_exact<R, F>(parser: F) -> impl Fn(&str) -> Result<R>
where
    F: Fn(&str) -> Result<R>,
{
    map_info(parser, |info| {
